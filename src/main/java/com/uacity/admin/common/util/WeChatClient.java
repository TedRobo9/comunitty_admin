...
import com.uacity.admin.domain.Chatter;
import com.uacity.admin.domain.Follower;
import com.uacity.admin.domain.Oauth;
...
public class WeChatClient {
    ...
    public static Boolean initAccessToken() {
        ...
        if (responseJSONObject != null && responseJSONObject.has("access_token")) {
            ...
            logger.info("access_token:"+responseJSONObject.getString("access_token"));
            return true;
        }
        return false;
    }
...
//        File menuFile = new File(System.getProperty("webapp.root"), "public/wechat.menu");
        String str = MessageUtil.getMessage("menu");
        JSONObject responseJSONObject = HttpClient.post("https://api.weixin.qq.com/cgi-bin/menu/create").data("access_token", accessToken).content(str).toJSONObject();
        logger.info(responseJSONObject.getString("errcode"));
        logger.info(responseJSONObject.getString("errmsg"));
